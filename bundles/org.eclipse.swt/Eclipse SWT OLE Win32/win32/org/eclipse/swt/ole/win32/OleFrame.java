			...
			if (lastFocus[0] != currentFocus) {
				...
				if (lastFocus[0] instanceof OleFrame) {
					if (!lastFocus [0].isDisposed()) {
						...
						Control[] children = frame.getChildren();
						for (int i = 0; i < children.length; i++){
							children[i].notifyListeners(SWT.FocusOut, event);
						}
					}
				}
...
private void createCOMInterfaces() {
	...
	iOleInPlaceFrame = new COMObject(new int[]{2, 0, 0, 1, 1, 1, 1, 1, 2, 2, 3, 1, 1, 1, 2}){
		public int method0(int[] args) {System.out.println("OleFrame.QueryInterface");return QueryInterface(args[0], args[1]);}
		public int method1(int[] args) {System.out.println("OleFrame.AddRef");return AddRef();}
		public int method2(int[] args) {System.out.println("OleFrame.Release");return Release();}
		public int method3(int[] args) {System.out.println("OleFrame.GetWindow");return GetWindow(args[0]);}
		public int method4(int[] args) {System.out.println("OleFrame.ContextSensitiveHelp");return ContextSensitiveHelp(args[0]);}
		public int method5(int[] args) {System.out.println("OleFrame.GetBorder");return GetBorder(args[0]);}
		public int method6(int[] args) {System.out.println("OleFrame.RequestBorderSpace");return RequestBorderSpace(args[0]);}
		public int method7(int[] args) {System.out.println("OleFrame.SetBorderSpace");return SetBorderSpace(args[0]);}
		public int method8(int[] args) {System.out.println("OleFrame.SetActiveObject");return SetActiveObject(args[0], args[1]);}
		public int method9(int[] args) {System.out.println("OleFrame.InsertMenus");return InsertMenus(args[0], args[1]);}
		public int method10(int[] args) {System.out.println("OleFrame.SetMenu");return SetMenu(args[0], args[1], args[2]);}
		public int method11(int[] args) {System.out.println("OleFrame.RemoveMenus");return RemoveMenus(args[0]);}
		// method12 SetStatusText - not implemented
		// method13 EnableModeless - not implemented
		public int method14(int[] args) {System.out.println("OleFrame.TranslateAccelerator");return TranslateAccelerator(args[0], args[1]);}
	};
}
...
//	implements IUnknown, IOleInPlaceFrame, IOleContainer, IOleInPlaceUIWindow
	...
	if (COM.IsEqualGUID(guid, COM.IIDIUnknown) ||
	    COM.IsEqualGUID(guid, COM.IIDIOleWindow) ||
	    COM.IsEqualGUID(guid, COM.IIDIOleInPlaceSite) ||
	    COM.IsEqualGUID(guid, COM.IIDIOleInPlaceUIWindow) ||
	    COM.IsEqualGUID(guid, COM.IIDIOleInPlaceFrame) ) {
...
//	if (pborderwidths != 0) {
//		RECT rect = new RECT();
//		COM.MoveMemory(rect, pborderwidths, RECT.sizeof);
//	}
	return COM.S_OK;
}
...
private int SetBorderSpace(int pborderwidths) {
	...
	if (pborderwidths == 0) return COM.S_OK;
	RECT borderwidths = new RECT();
	COM.MoveMemory(borderwidths, pborderwidths, RECT.sizeof);
	Rectangle area = getClientArea();
	area.x += borderwidths.left;
	area.y += borderwidths.top;
	area.width -= borderwidths.left + borderwidths.right;
	area.height -= borderwidths.top +borderwidths.bottom;
	Control[] children = getChildren();
	for (int i = 0; i < children.length; i++) {
//		Rectangle indent =((OleClientSite)children[i]).getIndent();
//		children[i].setBounds(area.x + indent.x, area.y + indent.y, area.width - indent.x - indent.width, area.height - indent.y - indent.height);
		children[i].setBounds(area.x, area.y, area.width, area.height);
	}
	return COM.S_OK;
}
/**
...
private boolean translateOleAccelerator(MSG msg) {
	...
	System.out.println("translate result is "+result);
	return result != OLE.S_FALSE;
}
