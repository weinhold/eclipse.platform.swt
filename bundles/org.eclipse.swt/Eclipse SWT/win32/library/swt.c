...
JNIEXPORT jboolean JNICALL Java_org_eclipse_swt_internal_win32_OS_IsDialogMessage
	(JNIEnv *env, jclass that, jint hDlg, jobject msg)
{
	DECL_GLOB(pGlob)
	
	MSG _msg, *lpMsg=NULL;

	DEBUG_CALL("IsDialogMessage\n")
	if (msg) lpMsg = getMSGFields(env, msg, &_msg, &PGLOB(MSGFc));
	
	return (jboolean)IsDialogMessage((HWND)hDlg, (LPMSG)lpMsg);
}